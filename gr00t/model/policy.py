...
class Gr00tPolicy(BasePolicy):
    ...
    def _load_metadata(self, exp_cfg_dir: Path):
        """Load dataset metadata and hand it to the modality transform."""

        # 1) 후보 파일: metadata.json 우선, 없으면 metadata_flat.json도 시도(선택)
        candidates = [exp_cfg_dir / "metadata.json", exp_cfg_dir / "metadata_flat.json"]
        metadata_path = None
        for p in candidates:
            if p.exists():
                metadata_path = p
        ...
        if metadata_path is None:
            raise FileNotFoundError(
                f"No metadata file found in {exp_cfg_dir}. "
                "Expected metadata.json (or metadata_flat.json)."
            )
        ...
        # 3) embodiment_tag 문자열로 정규화 (Enum/str 모두 허용)
        tag = getattr(self.embodiment_tag, "value", self.embodiment_tag)

        # 4) 포맷 정규화: 감싸진 포맷이면 언패킹, 이미 평탄화면 그대로 사용
        if isinstance(meta, dict) and "statistics" in meta and "modalities" in meta:
            inner = meta  # already flat
        elif isinstance(meta, dict) and tag in meta:
            inner = meta[tag]  # unwrap {"new_embodiment": {...}}
        else:
            raise ValueError(
                "Unexpected metadata format.\n"
                f"- Tried embodiment tag: '{tag}'\n"
                f"- Top-level keys: {list(meta.keys()) if isinstance(meta, dict) else type(meta)}\n"
                f"- File: {metadata_path}"
            )

        # 5) 모델 검증 & 주입
        metadata = DatasetMetadata.model_validate(inner)
